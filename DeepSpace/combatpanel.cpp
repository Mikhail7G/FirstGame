...
combathud::combathud(player *_plr)
{
	plr = _plr;
...
void combathud::PlayerPanel()
{
	auto pship = plr->PlayerShip;

	back->RenderEx(panx, pany, 0, 0.6, 0.6);
...
void combathud::WeaponOperations()
{
	auto pship = plr->PlayerShip;

	for (int i = 0; i<pship->weapons.size(); i++)//рисуем иконки орудий в слотах по бокам
	{
...
void combathud::TargetPanel()
{
	...
	if (!pship->target.expired())//проверяем наличие цели
	{
		...
		ship *sp = dynamic_cast<ship*>(trg.get());//получаем корабль
		...
		case 1://для кораблей
			mainf->printf(panx1 + 120, pany1 - 280, HGETEXT_LEFT, trg->name);
			mainf->printf(panx1 + 120, pany1 - 265, HGETEXT_LEFT, "-%i-", sp->pilot->GetId());
...
bool combathud::MouseTest()
{
	hgeRect *rr = new hgeRect();//для левой
	hgeRect *rr1 = new hgeRect();//для правой
	back->GetBoundingBoxEx(panx, pany, 0, 0.6f, 0.6f, rr);
	back->GetBoundingBoxEx(panx1, pany1, 0, 0.6f, 0.6f, rr1);
	if ((rr->TestPoint(MouseX, MouseY)) || ((rr1->TestPoint(MouseX, MouseY))))
	{
		delete rr;
		delete rr1;
		return true;
	...
	//hgeRect *rr1 = new hgeRect();//для правой
	//back->GetBoundingBoxEx(panx1, pany1, 0, 1, 1, rr);
	//if (rr1->TestPoint(MouseX, MouseY))
	//{
	//	delete rr1;
	//	return true;
	//}
